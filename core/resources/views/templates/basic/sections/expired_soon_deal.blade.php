@php
    ...
    $coupons = \App\Models\Coupon::active()->with('store')->withCount(['reports' => function($report){
        $report->where('action', 'copy')->where('date', 'like', now()->format('Y-m-d'));
    }])->where('ending_date', '<=', now()->addDays(7) )->orderBy('ending_date', 'ASC')->limit(8);
...
<section class="pt-80 pb-80">
    <div class="container">
        <div class="section-header d-flex justify-content-between align-items-center gap-3">
            ...
            <a href="{{ route('coupon.filter.type', 'expire-soon') }}" class="btn btn--base btn-md flex-shrink-0">@lang('View All')</a>
        </div>
                    ...
                    <div class="coupon-item has--link">
                        ...
                        <div class="coupon-item__content">
                            ...
                            <div class="coupon-item__bottom">
                                ...
                                <div class="text--base" data-countdown="{{ $coupon->ending_date }}" data-title="expired">
                                </div>
                            </div>
                        </div>
                    </div>
